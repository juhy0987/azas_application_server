...
/**
 * 모든 API 호출용 fetch 래퍼.
 *  - localStorage 에 보관된 JWT 가 있으면 Authorization: Bearer <token> 자동 주입
 *  - 그 외 동작은 표준 fetch 와 동일
 */
...
/**
 * 401/403 응답(권한 없음) 여부를 확인하고, 해당 시 사용자에게 안내한다.
...
export async function fetchDocuments() {
  const res = await _fetch('/api/documents');
  if (!res.ok) throw new Error('Failed to fetch documents');
  return res.json();
}

export async function fetchDocument(documentId) {
  const res = await _fetch(`/api/documents/${documentId}`);
  if (!res.ok) throw new Error('Failed to fetch document');
  return res.json();
}

export async function apiCreateDocument() {
  const res = await _fetch('/api/documents', { method: 'POST' });
...
export async function apiDeleteDocument(documentId) {
  const res = await _fetch(`/api/documents/${documentId}`, { method: 'DELETE' });
  _checkPermission(res);
  if (!res.ok) throw new Error('Failed to delete document');
}

export async function apiPatchBlock(blockId, fields) {
  const res = await _fetch(`/api/blocks/${blockId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function apiCreateBlock(documentId, type, parentBlockId = null, targetDocumentId = null) {
  ...
  const res = await _fetch(`/api/documents/${documentId}/blocks`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function apiDeleteBlock(blockId) {
  const res = await _fetch(`/api/blocks/${blockId}`, { method: 'DELETE' });
  _checkPermission(res);
  if (!res.ok) throw new Error('Failed to delete block');
}

export async function apiChangeBlockType(blockId, type) {
  const res = await _fetch(`/api/blocks/${blockId}/type`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function apiUploadFile(file) {
  ...
  const res = await _fetch('/api/files', { method: 'POST', body: form });
  _checkPermission(res);
  if (!res.ok) {
...
export async function apiDeleteFile(fileId) {
  ...
  const res = await _fetch(`/api/files/${fileId}`, { method: 'DELETE' });
  _checkPermission(res);
...
export async function apiUploadImage(file) {
  const form = new FormData();
  form.append('file', file);
  const res = await _fetch('/api/upload', { method: 'POST', body: form });
  _checkPermission(res);
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
...
export async function apiFetchUrlEmbed(url, blockId = null) {
  ...
  const res = await _fetch('/api/url-embed/fetch', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
...
export async function apiMoveBlock(blockId, beforeBlockId) {
  const res = await _fetch(`/api/blocks/${blockId}/position`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function apiAddDbColumn(dbBlockId, name, type = 'text', options = []) {
  const res = await _fetch(`/api/database/blocks/${dbBlockId}/schema/columns`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function apiUpdateDbColumn(dbBlockId, colId, patch) {
  const res = await _fetch(`/api/database/blocks/${dbBlockId}/schema/columns/${colId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function apiUpdateDbRowProperties(dbRowBlockId, properties) {
  const res = await _fetch(`/api/database/blocks/${dbRowBlockId}/properties`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function apiPatchDatabaseBlock(dbBlockId, fields) {
  const res = await _fetch(`/api/database/blocks/${dbBlockId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(fields),
  });
...
export async function apiImportNotion(file) {
  const form = new FormData();
  form.append('file', file);
  const res = await _fetch('/api/import/notion', { method: 'POST', body: form });
  _checkPermission(res);
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
